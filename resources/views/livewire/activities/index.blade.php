    ...
    <div class="table-responsive-sm">
        ...
        <table class="table table-striped table-bordered align-middle">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <img height="50" src="{{asset('storage/'.$activity->image_small)}}" alt="">
                    </td>
                    <td>{{$activity->title}}</td>
                    <td>{{$activity->featured ? "Yes" : "No"}}</td>
                    <td>{{$activity->visible ? "Yes" : "No"}}</td>
                    <td>
                        <a href="{{route('activities.show', $activity->id)}}" class="btn bg-success text-white">View</a>
                        <a href="{{route('activities.edit', $activity->id)}}" class="btn bg-warning">Edit</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
