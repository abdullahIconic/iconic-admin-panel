    ...
    <div class="table-responsive-sm">
        ...
        <table class="table table-striped table-bordered align-middle">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <img height="50" src="{{asset('storage/'.$client->image_medium)}}" alt="">
                    </td>
                    <td>{{$client->name}}</td>
                    <td>{{$client->happy_for}}</td>
                    <td>{{$client->visible ? "Yes" : "No"}}</td>
                    <td>
                        <a href="{{route('happy-clients.show', $client->id)}}" class="btn bg-success text-white">View</a>
                        <a href="{{route('happy-clients.edit', $client->id)}}" class="btn bg-warning">Edit</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
