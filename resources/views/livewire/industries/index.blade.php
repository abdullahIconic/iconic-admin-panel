...
<div>
    <div class="mb-3">
        Total {{ $totalIndustries }} Industries
    </div>
    <div class="table-responsive-sm">
        ...
        <table class="table table-striped table-bordered align-middle">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <img height="50" src="{{asset('storage/'.$industry->image)}}" alt="">
                    </td>
                    <td>{{$industry->title}}</td>
                    <td>{{$industry->visible ? "Yes" : "No"}}</td>
                    <td>
                        <a href="{{route('industries.show', $industry->id)}}"
                            class="btn bg-success text-white">View</a>
                        <a href="{{route('industries.edit', $industry->id)}}" class="btn bg-warning">Edit</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
        </table>
        @if( method_exists($industries,'links') )
        {{ $industries->links() }}
        @endif
    </div>
</div>
