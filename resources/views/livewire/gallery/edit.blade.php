...
<form class="body" action="{{ route('gallery.update', $gallery->id) }}" method="post" enctype="multipart/form-data">
    @method('patch')
    @csrf
    ...
    <div class="row g-3">
        <div class="col-md-1">
            ...
            <select class="form-control @error('visible') is-invalid @enderror" name="visible" required>
                <option value="1" {{ $gallery->visible == 1 ? 'selected' : '' }}>Yes</option>
                <option value="0" {{ $gallery->visible == 0 ? 'selected' : '' }}>No</option>
            </select>
            @error('visible')
